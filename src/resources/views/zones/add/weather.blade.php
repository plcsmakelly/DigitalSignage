...
<div>
  <div class="md:grid md:grid-cols-3 md:gap-6">
    ...
    <form action="{{ url('zones/content/add/weather/'.$zone->id) }}" id="form" method="post">
    {!! csrf_field() !!}
    <div class="mt-5 md:mt-0 md:col-span-2">
	  ...
	  <div>
		...
		<div class="mt-1">
			<input type="text" id="station" class="form-input focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300" name="station" placeholder="KOFF" onkeypress="handleEnter(event)" autofocus>
		</div>
		<p class="mt-2 text-sm text-gray-500">
			Either the NWS <a href="https://forecast.weather.gov/stations.php?foo=0" target="_blank">station code</a>.  For Omaha, use KOMA.
              </p>
            </div>
